document.addEventListener('DOMContentLoaded', () => {
    fetch('http://localhost:3000/products/family/Cuatriciclos')
        .then(response => response.json())
        .then(data => {
            const contenedor = document.getElementById('lista-cuatriciclos');
        ...
        .catch(error => {
            console.error('Error al cargar cuatriciclos:', error);
        });
});