document.addEventListener('DOMContentLoaded', () => {
    fetch('http://localhost:3000/products/family/Equipo de buceo')
        .then(response => response.json())
        .then(data => {
            console.log(data);
            const contenedor = document.getElementById('lista-buceo');
        ...
        .catch(error => {
            console.error('Error al cargar los Equipo de buceo:', error);
        });
});